...
export default function Orb({
  ...
  useEffect(() => {
    ...
    const checkWebGLSupport = () => {
      try {
        ...
        // Check for software renderer (potential performance issues)
        ...
        if (debugInfo) {
          ...
          if (renderer && (
            ...
            console.warn('WebGL is using a software renderer:', renderer);
          }
        }
  ...
  return (
    <div ref={ctnDom} className="orb-container">
      {!webGLSupported && (
        <div className="orb-fallback" aria-label="Decorative orb background">
        </div>
      )}
      <div className="orb-text-overlay">
        <div className="orb-title-container">
          <TextType 
            text="AI Circle"
            className="orb-title"
            typingSpeed={80}
        ...
        <div className="orb-subtitle-container">
          <TextType 
            text="The expert network you need to build an exceptional career in AI"
            className="orb-subtitle"
            typingSpeed={40}
            initialDelay={800} // Start after title animation completes (9 chars * 80ms = 720ms + buffer)
            showCursor={false}
          />
        </div>
      </div>
